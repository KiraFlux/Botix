...

{
    ...
    struct Dependencies {
        Config const &config;
        IncomingTelemetry::ControlInputEntry const &control_input;
    };

    explicit constexpr MixerService(Dependencies const &deps) noexcept :
        kf::mixin::Configured<Config>{deps.config},
        _control_input{deps.control_input} {}
