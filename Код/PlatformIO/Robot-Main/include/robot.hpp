...
long calcPower(long error) {
  const int speed_kp = 8;
  return error * speed_kp;
}
...
void goTick(long L, long R, int speed) {
  ...
  do {
    left_error = L - left_current_position;
    right_error = R - right_current_position;

    left_speed = left_error * pos_kp;
    right_speed = right_error * pos_kp;

    updateMotors(constrain(left_speed, -speed, speed),
                 constrain(right_speed, -speed, speed));
  } while (not(isEncoderReady(left_error) and isEncoderReady(right_speed)));
...
void turnAngle(int angle, int speed) {
  ...
  long path_mm = angle * M_PI * track_width_mm / 360;
...
void lineDist(long distance_mm, int speed) {
  ...
  do {
    lineFollowRegulator(speed);
    error =
        left_current_position + right_current_position - double_target_position;
...
void robotSetup() {
  ...
  attachInterrupt(digitalPinToInterrupt(left_phase_a),
                  on_left_encoder_phase_a_rising, RISING);
  ...
  attachInterrupt(digitalPinToInterrupt(right_phase_a),
                  on_right_encoder_phase_a_rising, RISING);
}